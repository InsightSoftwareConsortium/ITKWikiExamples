...
int main(int, char *[])
{
  ...
  JoinImageFilterType::Pointer joinFilter
          = JoinImageFilterType::New ();
  joinFilter->SetInput1(image1);
  joinFilter->SetInput2(image2);
  joinFilter->Update();

  typedef itk::VectorImageToImageAdaptor<ImageType, 2> VectorImageAdaptorType;
  VectorImageAdaptorType::Pointer vectorImageAdaptor = VectorImageAdaptorType::New();
  vectorImageAdaptor->SetInput(joinFilter->GetOutput());
  ...
  ConnectorType::Pointer joinConnector = ConnectorType::New();
  joinConnector->SetInput(vectorImageAdaptor->GetOutput());

  vtkSmartPointer<vtkImageActor> joinActor =
    vtkSmartPointer<vtkImageActor>::New();
  joinActor->SetInput(joinConnector->GetOutput());
  ...
  rightRenderer->AddActor(joinedActor);
